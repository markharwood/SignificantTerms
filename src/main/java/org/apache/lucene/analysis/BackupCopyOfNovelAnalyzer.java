 ...
 * @author MAHarwood
 */
public final class BackupCopyOfNovelAnalyzer extends Analyzer
{
     ...
     * @param analyzer
     */
    public BackupCopyOfNovelAnalyzer(Analyzer analyzer, int minDupChainLength)
    {
    ...
    protected TokenStreamComponents createComponents(String fieldName, Reader reader)
    {
        ...
        return new NovelTokenStreamComponents(numTokenStreamsConsidered, delegate.createComponents(
                fieldName, reader));
    }
    ...
    class NovelTokenStreamComponents extends TokenStreamComponents
    {
        ...
        public NovelTokenStreamComponents(int docId, TokenStreamComponents delegateTokenStream)
        {
            super(delegateTokenStream.source);
            this.docId = docId;
            this.delegateTokenStream = delegateTokenStream.getTokenStream();
        ...
        public boolean incrementToken() throws IOException
        {
            ...
            if (validTokens.size() > 0)
            {
                ...
                source.restoreState(earlierToken);
                return true;
            }
            else
            {
        ...
        public void loadStack() throws IOException
        {
            ...
            while (delegateTokenStream.incrementToken())
            {
                totalNumInputTokens++;
                numInputTokensInLastTokenStream++;
                String currTermText = termAtt.toString();
                currToken = new ChainedToken(docId, lastChainedToken, source.captureState(),
                        currTermText);
                ...
                if (!stopWords.contains(currTermText))
                {
                    ...
                    if (prevOccs != null)
                    {
                        if (fastTrackToken == null)
                        {
                            ...
                            //                    only examine chains every now and then - avoids excessive comparison by looking back
                            //                    on every single word                      
                            //                          if(totalNumInputTokens%minDupChainLength==0) 
