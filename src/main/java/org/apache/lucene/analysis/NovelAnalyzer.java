...
/**
 * An Analyzer which only emits tokens for "novel" text ie sequences of words
 * not seen before. This has uses in:
 * <ol>
 ...
 * </ol>
 * The class prunes the information kept from multiple documents using a sliding
 * window of content which is used to search for duplicate text. Post-analysis
 * it is also possible to extract stats on the use of selected keywords in
 * phrases e.g. to find the relevant phrases for the top terms ["grand","f1",
 * "driver", "prix", "racing"]
 * 
 * TODO could live in a seperate project away from Significant Terms plugin
 * 
 * @author MAHarwood
 */
public final class NovelAnalyzer extends Analyzer {
    private Analyzer delegate;
    // TODO look at CharArrayMap (?) etc in core Lucene to avoid instantiating
    // Strings for terms?
            ...
            if (prevOccs != null) {
                ...
                for (ChainedToken tokenOcc : prevOccs) {
                    ...
                    while (keywords.contains(currToken.currToken)) {
                        ...
                        // I did experiment with allowing up to n words in a run
                        // which weren't explicitly listed in "keywords"
                        // in order to help include useful joining words (eg
                        // "of", "the" "and") alongside the statistically
                        // interesting keywords you might pass as keywords to
                        // this function.
                        // The quality of results wasn't too great in my tests
                        // so I would stick with the strict inclusion of only
                        // words in the keywords set into runs. If a client
                        // needs boring-but-useful connecting words in the runs
                        // they should be added to the keywords list (but this
                        // may match runs like "and the" so maybe there is 2
                        // tier
                        // system required where there is a set of mandatory
                        // keywords and a set of optional ones- the rule being a
                        // run must have at least one mandatory keyword in a
                        // run.
                    ...
                    if (chain.size() > 1) {
                        ...
                        if (uniqueDocIdsWithThisChain == null) {
                            // TODO a hashset maybe a little heavyweight here -
                            // we know num of docs parsed and only need a 1/0
                            // bit to mark presence of run in doc?
    ...
    private void prune() {
        ...
        for (List<ChainedToken> prevOccs : tokens.values()) {
            ...
            // fast exit - if last occurence is outside scope bin all occurences
            // and token from hashmap
            ...
            // See above comment. I have found values >=10 for
            // percentOfDocsInSet identifies
            // a useful set of stop words - may need to expose a property to
            // control this
            ...
            // prune stop words
                ...
                if (percentOfDocsInSet > stopWordThresholdFactor) {
                    ...
                    System.out.println("Stop-wording " + currToken.toString() + " " + percentOfDocsInSet);
                }
    ...
    public float getPercentDuplicateTermsInLastTokenStream() {
        return (float) numDuplicateTokensRemovedFromLastTokenStream / (float) numInputTokensInLastTokenStream;
    }
        ...
        public void loadStack() throws IOException {
            ...
            while (input.incrementToken()) {
                ...
                if (fastTrackToken != null) {
                    // we have a confirmed match (>minDupChainLength tokens in a
                    // row) and now focus
                    // exclusively on following this chain through until the end
                    if ((fastTrackToken.nextToken != null) && (currTermText.equals(fastTrackToken.nextToken.currToken))) {
                        ...
                        // trail has gone cold .. consider all alternatives here
                        // on in
                        fastTrackToken = null;
                    }
                }
                if (!stopWords.contains(currTermText)) {
                    ...
                    if (prevOccs != null) {
                        if (fastTrackToken == null) {
                            ...
                            // only examine chains every now and then - avoids
                            // excessive comparison by looking back on every
                            // single word
                            if ((totalNumInputTokens % checkEvery) == 0) {
                                // we have advanced sufficiently to now spend
                                // time taking stock of any overlaps
                                for (ChainedToken otherChainedToken : prevOccs) {
                                    ...
                                    while (thisChainedToken.currToken.equals(otherChainedToken.currToken)) {
                                        sameChainLength++;
                                    ...
                                    if (sameChainLength > thisTokensChainLength) {
                                        ...
                                        // if we have >5 shared tokens in a row
                                        // we are onto something - focus
                                        // exclusively
                                        // on testing this chain going forward
                                        // rather than current approach of
                                        // exhaustively backward testing all
                                        // chains that lead up to this token
            ...
            while (cursor != null) {
                if (ignoreCount <= 0) {
                    if (cursor.maxChainLength > minDupChainLength) {
                        ...
                        cursor.currTokenState = null; // remove state for
                                                      // emitted novel token
                    }
                } else {
                    cursor.currTokenState = null; // remove the state of this
                                                  // duplicate token
                    ignoreCount--;
    ...
    static class ChainedToken {
        ...
        public ChainedToken(int srcDocId, ChainedToken prevToken, State currTokenState, String currToken) {
